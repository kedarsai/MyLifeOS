...
def threads_list(request: Request, goal_id: str | None = Query(default=None)) -> Any:
    settings = request.app.state.settings
    apply_sql_migrations(_project_root(), settings)
    items = list_chat_threads(settings, goal_id=goal_id)
...
async def threads_create(request: Request) -> Any:
    ...
    item = create_chat_thread(settings, title=payload.title, goal_id=payload.goal_id)
    if wants_html(request):
...
def chat_context(request: Request, goal_id: str | None = Query(default=None)) -> Any:
    settings = request.app.state.settings
    apply_sql_migrations(_project_root(), settings)
